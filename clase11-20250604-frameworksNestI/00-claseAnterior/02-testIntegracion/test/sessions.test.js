...
describe("Prueba funcional router sessions", async function () {
    ...
    it("Si ejecuto la ruta current y envio una cookie, me retorna el nombre del usuario", async () => {

        let { body } = await requester.get("/api/sessions/current").set("Cookie", cookie)

        expect(body).to.has.property("payload")
        expect(body.payload.email).to.be.ok
        expect(body.payload.email).to.be.eq(this.userMock.email)
    })

    it("Si ejecuto /api/sessions/login, metodo POST, enviando un usuario ya resgistrado, pero con password incorrecta, retorna status code http 400", async () => {
        this.userMock.password="12345"
        let { status } = await requester.post("/api/sessions/login").send(this.userMock)

        expect(status).to.be.eq(400)
    })

    it("Si ejecuto /api/sessions/login, metodo POST, enviando un usuario ya resgistrado, pero con password incorrecta, retorna objeto con la propiedad status con valor error", async () => {
        let { body } = await requester.post("/api/sessions/login").send(this.userMock)

        expect(body.status).to.be.eq("error")
    })

    it("Si ejecuto /api/sessions/login, metodo POST, enviando un usuario ya resgistrado, pero con password incorrecta, retorna objeto con la propiedad error con valor 'Incorrect password'", async () => {
        let { body } = await requester.post("/api/sessions/login").send(this.userMock)
